function optPath=computeOptPathCase1(currentstate,currentPstate,T,P,H,...
    currentpath,lastPrevPstate,optPath,K,t)
...
%         fprintf('lastPrevPstate Energy: %2.2f, lastCurrentPstate energy of this path: %2.2f\n',...
%             P.cost(lastPrevPstate),P.cost(currentPstate));
...
%% Recursive call with cycle detection %%
    for ii=find(P.trans(currentPstate,:))
        if isempty(find(currentpath.Ppath==ii)) && P.cost(ii)~=inf
            ...
            optPath=computeOptPathCase1(nextstate,nextPstate,T,P,H,currentpath,lastPrevPstate,optPath,K,t);
        end
    end
end